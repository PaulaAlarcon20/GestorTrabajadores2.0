...
class RegisterCubit extends Cubit<RegisterState> {
  ...
  Future<void> onSubmit() async {
    ...
    emit(
      // Notifica a flutter blog que el estado ha cambiado
      state.copyWith(
          // hace una copia del estado actual, pero con algunos valores modificados
    ...
    try {
      final url = Uri.parse('http://localhost:8080/api/usuarios/login');
  ...
  void emailChanged(String value) {
    ...
    emit(
        // Función utilizada en Cubit para notificar a Flutter de que el estado ha cambiado

        state.copyWith(
      // Crea nueva instancia manteniendo el estado anterior y solo cambia el campo email
      email: emailNewValue,
      isValid: Formz.validate([
        emailNewValue,
        state.password
