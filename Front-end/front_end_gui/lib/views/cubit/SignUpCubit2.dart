...
class SignUpCubit2 extends Cubit<SignUpState2> {
  SignUpCubit2()
      : super(const SignUpState2(
            isValid: false, isValid2: false, isValid3: false, avance: false));
  ...
  void onSubmit(int parte) {
    ...
    final newState = state.copyWith(
      ...
      preferenciasHorarias: PreferenciasRadioButton.dirty(
          value: state.preferenciasHorarias.value),
      disponibilidadHorasExtras: DisponibilidadHorasExtras.dirty(
          value: state.disponibilidadHorasExtras.value),
    );
  ...
  Future<void> sendHttpPost() async {
    ...
    try {
      //1- Almacenar URL api
      final url = Uri.parse(
          'http://localhost:8080/api/usuarios/SignUp'); //TODO va a haber que cambiar el mapping del futuro @get
  ...
  Future<void> getUsuariosPorCorreo(String email) async {
    final Uri uri =
        Uri.parse("http://localhost:8080/api/usuarios/DataUser?email=$email");
  ...
  void nombreChanged(String nombre) {
    ...
    emit(state.copyWith(
        ...
        isValid: Formz.validate([
          ...
          state.gmail,
          state.telefono
        ]) // le vamos a tener que enviar todo los campos porque necesita saber si todos los campos son validos
        ));
  }
  ...
  void apellidosChanged(String apellidos) {
    ...
    emit(state.copyWith(
        ...
        isValid: Formz.validate(
            [apellidosNewValor, state.nombre, state.gmail, state.telefono])));
  }
  ...
  void gmailChanged(String email) {
    ...
    emit(state.copyWith(
        gmail: gmailNewValor,
        isValid: Formz.validate(
            [gmailNewValor, state.nombre, state.apellidos, state.telefono])));
  }
  ...
  void telefonoChanged(String telefono) {
    ...
    emit(state.copyWith(
        ...
        isValid: Formz.validate(
            [telefonoNewValor, state.nombre, state.apellidos, state.gmail])));
  }
  ...
  void preferenciaHorariaChanged(String preferencias) {
    ...
    final preferenciasHorariasNewValor =
        PreferenciasRadioButton.dirty(value: preferencias);

    emit(state.copyWith(
        preferenciasHorarias: preferenciasHorariasNewValor,
        isValid2: Formz.validate([
          preferenciasHorariasNewValor,
          state.centroDeTrabajo,
          state.puesto,
          state.localidad,
          state.disponibilidadHorasExtras
  ...
  void disponibilidadHorasExtrasChanged(int disponibilidadHorasExtras) {
    final disponibilidadhorasExtrasNewValor =
        DisponibilidadHorasExtras.dirty(value: disponibilidadHorasExtras);
    log('SE EJECUTA EL METODO DEL READIO BUTTON');
    emit(state.copyWith(
        disponibilidadHorasExtras: disponibilidadhorasExtrasNewValor,
        isValid2: Formz.validate([
          disponibilidadhorasExtrasNewValor,
          state.puesto,
          state.localidad,
          state.centroDeTrabajo,
          state.preferenciasHorarias
  ...
  void passwordChanged(String password) {
    ...
    emit(state.copyWith(
        ...
        isValid3: Formz.validate([
          passwordNewValor
        ]) // TODO validador nuevo puede hacer falta, vamos a probar
        ));
  }
}
