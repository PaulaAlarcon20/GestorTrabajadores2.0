...
class Peticioneshttp {
  ...
  Future<bool> cerrarSesion({required email, required inicioSesion}) async {
    try {
      final url = Uri.parse('http://localhost:8080/api/usuarios/LogOut');

      final response = await http.post(url,
          headers: {'Content-Type': 'application/json'},
          body: jsonEncode({'email': email, 'inicio_sesion': inicioSesion}));

      if (response.statusCode == 200) {
        print(
            "Se envia al backend: $email y valor inicioSesion: $inicioSesion");
        ...
        print(
            "Se ha intentado enviar al backend: $email y valor inicioSesion: $inicioSesion");
        return true;
      }
    } catch (e, stackTrace) {
